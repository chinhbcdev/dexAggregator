...
const from_Value = 5000000000000000;
async function getQuote(account) {
  ...
  document.getElementById("from_amount").value =
    from_Value / 10 ** currentTrade.to.decimals;
  if (
    !currentTrade.from ||
    !currentTrade.to ||
    !document.getElementById("from_amount").value
  ...
  //   );
  const response = await fetch(
    `https://goerli.api.0x.org/swap/v1/quote?sellToken=${sellToken}&buyToken=${buyToken}&sellAmount=${sellAmount}`
    //{
    //   method: 'GET',
    //   headers: [
    //     ['0x-api-key', '6b321e0f-8d29-45de-ac49-ef0726c2b4cc']
...
async function trySwap() {
  ...
  //onst web3 = new Web3(Web3.givenProvider);
  ...
  const receipt = await web3.eth.sendTransaction({
    ...
    value: from_Value,
    gasLimit: 580000,
    // gasPrice: web3.utils.fromWei("100", "gwei"),
  });
